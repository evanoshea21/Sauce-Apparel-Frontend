...
function ProductRow({ product, row, refreshRow }: ProductRowProps) {
  ...
  return (
    ...
    >
      ...
      <p>Flavors:</p>
      <FlavorsCrud refreshRow={refreshRow} product={product} />
      <div>
        <img src={product.imageUrl} width="60" height="60" alt="image" />
      </div>
...
function FlavorsCrud({ product, refreshRow }: FlavorsProps) {
  ...
  function addFlavorsAndStocks(e: any) {
    ...
    // parse flavors and stock counts
    let flavorsToAdd = flavorsString.split(",");
    let stocksToAdd = inventoryString.split(",");
    // use product as default payload to send
    let payload = {
      ...product,
    };
    // replace flavors & stocks
